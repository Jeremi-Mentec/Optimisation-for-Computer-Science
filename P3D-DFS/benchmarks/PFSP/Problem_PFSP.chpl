module Problem_PFSP
{
  ...
  const allowedLowerBounds = ["lb1", "lb1_d", "lb2","lb_mix"];
  ...
  class Problem_PFSP : Problem
  {
    ...
    proc init(const fileName: string, const lb: string, const rules: string, const ub: string, const depthThreshold: real, const firstLb: bool): void
    {
      this.name = fileName;
      this.depthThreshold = depthThreshold;
      this.firstLb = firstLb;
    ...
    override proc copy()
    {
      return new Problem_PFSP(this.name, this.lb_name, this.branching, this.ub_init, this.depthThreshold, this.firstLb);
    }
      ...
      ref max_depth: int, ref best: int, lock: sync bool, ref best_task: int): list(?)
    {
      select this.lb_name {
        ...
        when "lb_mix"{
          if (parent.depth <= this.jobs*this.depthThreshold ){
            if (firstLb){
              return decompose_lb1_d(Node, parent, tree_loc, num_sol, max_depth, best, lock, best_task);
            }
            else {
      ...
      const subDepthReached: [] int, const best: int, const elapsedTime: real): void
    {
      ...
      writeln("depthThreshold: ", depthThreshold);
      writeln("=================================================\n");
    }
